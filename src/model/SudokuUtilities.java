...
/**
 * Represents the utilities needed in order to create the logic in a Sudoku game
 */
public class SudokuUtilities {
    ...
    static int[][][] convertStringToIntMatrix(String stringRepresentation) {
        ...
        //char[] charRepresentation = startingPoint(stringRepresentation).toCharArray();
    ...
    private static int[][][] startingPoint(int[][][] values) {
        Random rand=new Random();
        int x = rand.nextInt(3);
        if(x == 0)
            return flipHorizontally(values);
        else if (x==1)
            return flipVertically(values);
        else return values;

    }

    private static int[][][] flipHorizontally(int[][][] values) {
        for (Object[] row : values) {  // Extra for loop to go through each row in turn, performing the reversal within that row.
    ...
    private static int[][][] flipVertically(int[][][] values) {
